  ...
  parfor SNRnum = 1:length(SNR_list),
    ...
    for noisenum = 1:length(noise_list),
      ...
      for iter = 1:SPLIT_SIZE,
          % disp(['proc: SNR = ' num2str(SNR) ', No' sprintf('%04d',(noisenum-1)*SPLIT_SIZE+iter-1) '...'])
          N_mic=2;
          FileDir = ['../anechoic_work_2mic/ch1/test_data/No' sprintf('%05d',train_size+(SNRnum-1)*length(noise_list)*SPLIT_SIZE+(noisenum-1)*SPLIT_SIZE+iter) '.wav'];
          signal1 = audioread([FileDir]);
          FileDir = ['../anechoic_work_2mic/ch2/test_data/No' sprintf('%05d',train_size+(SNRnum-1)*length(noise_list)*SPLIT_SIZE+(noisenum-1)*SPLIT_SIZE+iter) '.wav'];
          signal2 = audioread([FileDir]);
          ...
          % y_out1=Synth(Mask(:,:,1).*X(:,:,1),synparam,300,5500);
          y_out1=Synth(Mask11.*X(:,:,1),synparam,300,5500);
          audiowrite([FileDir2 'No' sprintf('%05d',train_size+(SNRnum-1)*length(noise_list)*SPLIT_SIZE+(noisenum-1)*SPLIT_SIZE+iter) '.wav'],y_out1 ,16000);
          % y_out2=Synth(Mask(:,:,2).*X(:,:,1),synparam,300,5500);
          % y_out2=Synth(Mask12.*X(:,:,1),synparam,300,5500);
          % audiowrite([FileDir2 'noisy_No' sprintf('%04d',iter-1) '.wav'],y_out2 ,16000);
